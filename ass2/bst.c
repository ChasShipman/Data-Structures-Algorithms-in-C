	...
	while(1){
		...
		while (nodeCount > 0){
			...
			if(tempNode->left == 0 && tempNode->right == 0)
				fprintf(fp, "=");
			t->display(tempNode->value, fp);
				fprintf(fp, "(");//print parent dec
			t->display(tempNode->parent->value, fp);//
				fprintf(fp, ")");//
			if(tempNode->parent == tempNode) //denote parent X dec
				fprintf(fp,"X");//
			else if(tempNode->parent->left == tempNode)//denote LEFT L dec
				fprintf(fp, "L");//
			else{
				if (tempNode->parent->right == tempNode)//denote RIGHT R dec
				fprintf(fp, "R");//
			}
			dequeue(levelOrder);
			if(tempNode->left)
